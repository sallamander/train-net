...
class TestTrainingJob():
    ...
    def _check_instantiate_dataset(self, training_job, set_name, monkeypatch):
        ...
        monkeypatch.setattr(
            'trainet.training.tf.training_job.import_object',
            mock_import_object
        )
        ...
        monkeypatch.setattr(
            'trainet.training.tf.training_job.AugmentedDataset',
        ...
        mock_loader = create_autospec(
            DataLoader, augmented_dataset=mock_augmented_dataset
        )
        ...
        MockDataLoader.assert_called_once_with(
            'return_from_augmented_dataset_init',
        )
    ...
    def test_instantiate_dataset__errors(self):
        ...
        - When `set_name==train` and there is no `fpath_df_train` in the
          `TrainingJob.config['dataset']` (RunTimeError)
        """
