...
from trainet.training.tf.data_loader import format_batch, DataLoader
...
class TestDataLoader():
    ...
    def _get_batches(self, data_loader):
        """Return two batches of data from the provided `data_loader`

        :param data_loader: dataset to return batches from
        :type data_loader: trainet.training.tf.data_loader.DataLoader
        :return: batches of data
        :rtype: list[numpy.ndarray]
        """

        with tf.device('/cpu:0'):
            dataset = data_loader.get_infinite_iter(
                self=data_loader, batch_size=self.BATCH_SIZE, n_workers=0
            )
    ...
    def test_init(self):
        ...
        data_loader = DataLoader(augmented_dataset=augmented_dataset)
        assert id(data_loader.augmented_dataset) == id(augmented_dataset)
    ...
    def test_get_infinite_iter(self):
        ...
        augmented_dataset.sample_types = {
            'element': 'float32', 'label': 'int16'
        }
        ...
        data_loader.augmented_dataset = augmented_dataset
        data_loader.get_infinite_iter = DataLoader.get_infinite_iter

        batches = self._get_batches(data_loader)
